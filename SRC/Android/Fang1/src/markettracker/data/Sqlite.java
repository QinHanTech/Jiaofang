...
public class Sqlite {
	...
	public synchronized static void execSqlList(Context context,
			List<String> SqlList) {
	...
	public synchronized static String addDate(Context context, String date,
			String days) {
		String data;
		try {
	...
	public synchronized static FieldsList getReportList(Context context,
			Fields condition) {
		FieldsList list;
		try {
	...
	public synchronized static List<String> getTemplateIdList(Context context,
	...
	public synchronized static boolean upsertData(Context context,
			QueryDataResult queryDataResult) {
		try {
			if (getDB(context).upsertData(queryDataResult) < 0)
				return false;
		} catch (Exception ex) {
			return false;
		}
	...
	public synchronized static FieldsList getFieldsList(Context context,
	...
	public synchronized static FieldsList getFieldsListBySql(Context context,
			String sql) {
		FieldsList list = new FieldsList();
		try {
			list = getDB(context).getFieldsListBySql(sql);
		} catch (Exception ex) {
			return null;
		}
		return list;
	...
	public synchronized static List<Louceng> getLouceng(Context context,
			String ldid) {
		List<Louceng> list = new ArrayList<Louceng>();
		try {
			list = getDB(context).getLouceng(ldid);
		} catch (Exception ex) {
			return null;
		}
		return list;
	...
	public synchronized static SObject getReport(Context context,
			Template temp, String code, int type, String key) {
		SObject object;
		try {
	...
	public synchronized static SObject getDPReport(Context context,
			Template temp, String code) {
		SObject object;
		try {
	...
	public synchronized static SObject getSurveyRpt(Context context,
			Template temp, String code) {
		SObject object;
		try {
	...
	public static List<DicData> getDictDataList(Context context,
			String mainDictId, String detaildicId) {
		try {
